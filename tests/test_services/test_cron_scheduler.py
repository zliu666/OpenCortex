...
from opencortex.services.cron_scheduler import (
    _jobs_due,
    append_history,
    execute_job,
    load_history,
...
def _tmp_dirs(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    data_dir.mkdir()
    logs_dir.mkdir()
    monkeypatch.setattr("opencortex.services.cron_scheduler.get_data_dir", lambda: data_dir)
    monkeypatch.setattr("opencortex.services.cron_scheduler.get_logs_dir", lambda: logs_dir)
...
class TestExecuteJob:
    ...
    async def test_timeout_job(self) -> None:
        with patch("opencortex.services.cron_scheduler.asyncio.wait_for") as mock_wait:
            import asyncio
            ...
            with patch(
                "opencortex.services.cron_scheduler.asyncio.create_subprocess_exec",
...
class TestSchedulerLoop:
    ...
    async def test_once_mode_fires_due_job(self) -> None:
        ...
        from opencortex.services.cron import upsert_cron_job

        upsert_cron_job({"name": "test-once", "schedule": "* * * * *", "command": "echo fired"})

        # Force next_run to the past so it's immediately due
        from opencortex.services.cron import load_cron_jobs, save_cron_jobs
